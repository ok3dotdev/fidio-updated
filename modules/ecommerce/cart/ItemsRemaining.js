...
const Module = props => {
  ...
  }, props?.useCartOfCurrency?.remaining?.length > 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#222222',
      borderRadius: '.5rem',
      fontSize: '.75rem',
  ...
  }, "The Items below will be processed after the current purchase. They are in a different currency.") : null, props?.useCartOfCurrency?.remaining?.map ? props?.useCartOfCurrency?.remaining?.map((item, i) => {
