...
import {
  ...
  OrderImage,
  OrderCard,
  OrderViewReceiptPage,
} from '/modules/ecommerce/receipt/order';
          ...
          <div className='Ecommerce_SingleOrderInternalContainer'>
            <div className='Ecommerce_SingleOrderMetaContainer'>
              <div className='Ecommerce_SingleOrderMeta'>
                ...
                &nbsp;-&nbsp;
                <span
                  className='Ecommerce_Order_SelectDate'
                  selectValue={`${selectDate ?? ''}`}
                  onClick={selectThisText}
                >{`${selectDate ?? ''}`}</span>
                <OrderViewReceiptPage {...props} m={m} />
              </div>
              <div className='Ecommerce_SingleOrderMetaCreation'>
                {creation ?? ''}
              </div>
            </div>
            <div className='Ecommerce_OrderSingleItemContainer'>
              ...
              resolveRegionBasedPrice
                ? props.m.cart.map((n, j) => {
                    const currentStyle = resolveCurrentStyle(
                      n.product,
                      n.style
                    );
                    const currentOption = resolveCurrentOption(
                      currentStyle,
                      n.option
                    );
                    const priceObject = resolveRegionBasedPrice(
                      props,
                      currentStyle
                    );
                    ...
                    return (
                      <div className='Ecommerce_Item_Container' key={j} i={j}>
                        <div className='Ecommerce_Item_ContainerInternal'>
                          <OrderImage
                            n={n}
                            resolveOrderImg={props?.resolveOrderImg}
                          />
                          <div className='Ecommerce_ItemContainerWidth'>
                            ...
                            <div className='flex Ecommerce_Order_StyleOrderContainer'>
                              ...
                              <div
                                className={`Ecommerce_Cart_Side_Meta_Container Ecommerce_Price_Expanded_Container`}
                              >
                                <div
                                  className={`Ecommerce_Order_SubtotalLabel`}
                                >
                                  Subtotal:
                                </div>
                                <div className='Ecommerce_Price'>
                                  {symbol}
                                  {resolveMoneyFormat(n.quantity * n.price)}
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            {m?.totals?.totalNoTax ? (
              <div
                className='flex Price_ItemsTotal'
                style={{ fontSize: '.85rem' }}
              >
                <div className='Ecommerce_Price_Label'>Before Tax:&nbsp;</div>
                <div className='Ecommerce_Price_Price'>
                  {useSymbol ?? null}
                  {resolveMoneyFormat(m.totals.totalNoTax)}
                </div>
              </div>
            ) : null}
            {m?.totals?.tax ? (
              <div className='flex Price_Tax' style={{ fontSize: '.85rem' }}>
                ...
                <div className='Ecommerce_Price_Price'>
                  {useSymbol ?? null}
                  {resolveMoneyFormat(m.totals.tax)}
                </div>
              </div>
            ) : null}
            <div
            ...
            >
              ...
              <div style={{ fontSize: '1.25rem' }}>
                {useSymbol}
                {westernMoneyFormat.format(m?.totals?.total) ?? ''}{' '}
